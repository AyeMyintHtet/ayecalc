...
export default function ImageCropper() {
  ...
  useEffect(() => {
    return () => {
      ...
      if (itemRef.current) {
        ...
        if (itemRef.current.result)
          URL.revokeObjectURL(itemRef.current.result.previewUrl);
      }
    };
  }, []);
  ...
  const rotatedDimensions = item
    ...
    : { width: 1, height: 1 };
  const activeRatio =
    ratios.find((ratio) => ratio.id === ratioId)?.value ?? null;
  ...
  function replaceResult(
    nextResult?: ImageProcessingResult & { previewUrl: string },
  ) {
    setItem((current) => {
      ...
      return {
        ...current,
        result: nextResult,
        status: nextResult ? "complete" : "ready",
      };
    });
  }
  ...
  function updateCrop(next: Partial<CropTransform>) {
    ...
    setTransform((current) => {
      ...
      const width = Math.max(
        1,
        Math.min(merged.width, rotatedDimensions.width),
      );
      const height = Math.max(
        1,
        Math.min(merged.height, rotatedDimensions.height),
      );
      return {
        ...merged,
        width,
  ...
  function chooseRatio(nextRatioId: RatioId) {
    ...
    invalidateResult();
    const ratio =
      ratios.find((candidate) => candidate.id === nextRatioId)?.value ?? null;
    const dimensions = getRotatedDimensions(
      item.width,
      item.height,
      transform.rotation,
  ...
  function rotateImage() {
    if (!item || isProcessing) return;
    invalidateResult();
    const rotation = ((transform.rotation + 90) %
      360) as CropTransform["rotation"];
  ...
  function movePointer(event: PointerEvent<HTMLDivElement>) {
    ...
    const deltaX =
      ((event.clientX - drag.startX) / bounds.width) * rotatedDimensions.width;
    const deltaY =
    ...
    if (activeRatio) {
      if (Math.abs(deltaX) >= Math.abs(deltaY * activeRatio))
        height = width / activeRatio;
      else width = height * activeRatio;
    }

    const maxWidth = isWest
      ? drag.crop.x + drag.crop.width
      : rotatedDimensions.width - drag.crop.x;
    const maxHeight = isNorth
      ? drag.crop.y + drag.crop.height
      : rotatedDimensions.height - drag.crop.y;
  ...
  function handleCropKey(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if (event.key === "ArrowLeft") updateCrop({ x: transform.x - amount });
      ...
      updateCrop({ x: transform.x + amount });
    ...
    <div className={styles.toolCard}>
      ...
      {!item ? (
        ...
        >
          ...
          <p>
            JPEG, PNG, or WebP · up to {formatImageBytes(MAX_IMAGE_FILE_BYTES)}{" "}
        ...
        <>
          <div className={styles.cropWorkspace}>
            ...
            <fieldset
              className={styles.cropControlsPanel}
              disabled={isProcessing}
            >
              ...
              <label className={`${styles.field} ${styles.rangeField}`}>
                <span>
                  Editor zoom <b>{transform.zoom.toFixed(1)}×</b>
                </span>
                <input
                  type="range"
                  min="1"
              ...
              <label className={styles.field}>
                ...
                >
                  ...
                  <option
                    value="image/jpeg"
                    disabled={!supportedOutputs.includes("image/jpeg")}
                  >
                    JPEG
                  </option>
                  ...
                  <option
                    value="image/webp"
                    disabled={!supportedOutputs.includes("image/webp")}
                  >
                    WebP
                  </option>
                </select>
          ...
          {item.result && (
            <div className={styles.cropOutputPreview}>
              ...
              <figure>
                <figcaption>
                  Crop · {item.result.width} × {item.result.height}px
                </figcaption>
                <img
                  src={item.result.previewUrl}
                  alt={`Cropped ${item.file.name}`}
                />
              </figure>
            </div>
          )}
          ...
          >
            ...
            {isProcessing && (
              <div
                ...
                role="progressbar"
                aria-label="Crop export progress"
                aria-valuemin={0}
                aria-valuemax={100}
          ...
          <div className={styles.actions}>
            {item.result ? (
              ...
              <button
                type="button"
                className={styles.primaryButton}
                disabled={isProcessing || hasUnsupportedOutput}
                onClick={exportCrop}
              >
                {isProcessing ? "Creating crop…" : "Create cropped image"}
              </button>
            )}
            <button
              type="button"
          ...
          <p className={styles.privacyNote}>
            The image is processed locally. Export removes EXIF and other
            embedded metadata.
          </p>
        </>
      )}
